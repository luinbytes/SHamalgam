...
- Signature-backed interface lookups checked: 6 missing on native Linux TF2
...
`Get_TFPartyClient` now uses a native Linux `client.so` getter signature.
...
`StaticPropMgr` is resolved on Linux through the native `StaticPropMgrClient004`
interface with the verified `CStaticPropMgr` base adjustment.
...
surface, while the remaining byte signatures still need native Linux
replacement work.
